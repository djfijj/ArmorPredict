...
static uint8_t get_blob_color(const cv::Mat &src, const cv::RotatedRect &blobPos) {
	...
	region &= cv::Rect(0, 0, src.cols, src.rows);   //cols列，rows行
...
bool ArmorFinder::findLightBlobs(const cv::Mat &src, LightBlobs &light_blobs) {
	...
	namedWindow("参数调试器", 1);
	createTrackbar("阈值light 100", "参数调试器", &light_threshold_slider_light, 100);
	createTrackbar("阈值dim 100", "参数调试器", &light_threshold_slider_dim, 100);*/
	//createTrackbar("阈值dim 100", "bin_light", &light_threshold_slider_dim, 100);
	...
	imagePreProcess(src_bin_light);                                  // 形态学运算

	cv::threshold(color_channel, src_bin_dim, light_threshold0, 255, CV_THRESH_BINARY); // 二值化对应通道（二值化参数选择？）
	if (src_bin_dim.empty()) return false;
	imagePreProcess(src_bin_dim);                                  // 形态学运算
		...
		if (hierarchy_dim[i][2] == -1) {
			...
			if (isValidLightBlob(light_contours_dim[i], rect)) {
				light_blobs_dim.emplace_back(
					rect, areaRatio(light_contours_dim[i], rect), get_blob_color(src, rect)   //插入
					);
			}
		}
	}
	...
	sort(light_to_remove.begin(), light_to_remove.end(), [](int a, int b) { return a > b; });   //从大到小排序
